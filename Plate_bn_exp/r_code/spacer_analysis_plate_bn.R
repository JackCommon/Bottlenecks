...
#setwd("~/OneDrive - University of Exeter/Data/Bottlenecks") #Home OSX

crispr = read.csv('./data/colony_and_pfu_counts/all_cfu_pfu_t3_with_proportion.csv')
...
ggplot(crispr, aes(y=mean.CR2.spacers, x=bottleneck))+
  geom_boxplot()+
  labs(x='Bottleneck size', y="Average no. of CRISPR2 spacers")+
  theme_bw()

ggplot(crispr, aes(y=mean.CR1.spacers, x=bottleneck))+
  geom_boxplot()+
  labs(x='Bottleneck size', y="Average no. of CRISPR1 spacers")+
  theme_bw()

ggplot(crispr, aes(y=mean.total.spacers, x=bottleneck))+
  geom_boxplot()+
  labs(x='Bottleneck size', y="Average no. all CRISPR spacers")+
  theme_bw()
...
just = read.csv('./data/PCR/just_CRISPR_clones.csv')
just$bottleneck = as.factor(just$bottleneck)
cr1_just = subset(just, mean.CR1.spacers >= 1)
str(just)
just$bottleneck = relevel(just$bottleneck, ref='monoculture')
##### Summary figures to eyeball the data

# Boxplot to get an impression of the differences between bottleneck treatments
CR2_just_plot_phage_bn = ggplot(just, aes(y=mean.CR2.spacers, x=bottleneck))+
  geom_boxplot()+
...
total_just_plot_phage_bn = ggplot(just, aes(y=mean.total.spacers, x=bottleneck))+
